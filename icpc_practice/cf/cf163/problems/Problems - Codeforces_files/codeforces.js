    ...
    this.ping = function (pingUrl) {
        ...
        $.post(pingUrl, {ftaa: window._ftaa, bfaa: window._bfaa}, function (result) {
            ...
            for (var i = 0; i < result.length; ++i) {
                ...
                messages.push({name: result[i].name, text: text});
            }
...
//            window.localStorage[key] = defaultValue;
    ...
    this.setupEventBus = function () {
        var that = this;
        if (that.eventBus == null && window.codeforcesOptions.subscribeServerUrl) {
            info("Connecting to the pubsub server...");
            that.eventBus = new vertx.EventBus(window.codeforcesOptions.subscribeServerUrl, {protocols_whitelist: ["websocket", "xhr-streaming", "xdr-streaming", "xhr-polling", "xdr-polling", "iframe-htmlfile", "iframe-eventsource", "iframe-xhr-polling"], debug: true});
